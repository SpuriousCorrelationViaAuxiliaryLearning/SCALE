...
python train.py --pretrained_model --num_epochs 50 --method=$method --dataset=$dataset --spurious_strength=$spurious_strength --val_target_size=$val_target_size --weight_decay=$weight_decay --batch_size=$batch_size --init_lr=$init_lr --regularize_mode=$regularize_mode --group_size=$group_size --seed=$seed --data_dir=/hpctmp/e0200920/mcdominoes_data --output_dir=/hpctmp/e0200920/$method-$regularize_mode-$dataset-$spurious_strength-$val_target_size-$weight_decay-$batch_size-$init_lr-$group_size-$seed
